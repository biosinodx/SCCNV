...
parser=argparse.ArgumentParser(description="sccnv.py, v1.0.2, Xiao Dong, biosinodx@gmail.com, xiao.dong@einstein.yu.edu")
...
for i in range(0, len(dat6)):
	tmp=dat6[i][0:7]
	for j in range(0, len(dat6[0][7:])):
		pdf=[norm_pdf(x=dat6[i][7+j],mm=k,sg=sigma[j]) for k in range(0, 101)]
